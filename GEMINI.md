...
- **Always use import aliases** rather than relative imports (`./` or `../`) anywhere in this codebase:
  ...
  - For cross-package references, use the `@app/<package>` workspace alias (e.g., `import { db } from "@app/data"`, `import type { User } from "@app/data"`) or shorthand aliases (e.g., `@data`, `@backend`, `@docs`, `@hub`, `@store`, `@tools`, `@ui`).
