...
export { getAssetHeaders, getMimeType, isStandaloneMode };
 ...
 */
export async function serveMicroFrontend(req: Request, options: ServeMicroFrontendOptions): Promise<Response> {
		...
		if (firstSegment && embeddedFiles.some((f) => f.name?.startsWith(`mfes/${firstSegment}/`))) {
			...
			if (subPath) {
				const mfeAsset = embeddedFiles.find(
					(f) => f.name === `mfes/${firstSegment}/${subPath}` || f.name?.endsWith(`/${firstSegment}/${subPath}`),
				);
				if (mfeAsset) {
