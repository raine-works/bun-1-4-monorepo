...
import { CORS_HEADERS } from '@backend/lib/cors';
	...
	if (typeof fileOrHtml === 'string') {
		...
		for (let attempt = 0; attempt < 8; attempt++) {
			try {
				...
				if (html.length > 50 && (html.includes('</body>') || html.includes('<html') || html.includes('id="root"'))) {
					break;
				}
			} catch {}
			await Bun.sleep(25);
		...
		try {
			...
			for (const entry of entries) {
				...
				if (await indexFile.exists()) {
					...
					if (!html.includes('id="root"') && !html.includes('</body>') && !html.includes('<script')) {
						return false;
					}
